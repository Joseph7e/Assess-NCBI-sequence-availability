...
for n in open(nodes_file):
    ...
    mito_code = node_elements[8]

    mito_code_lookup[tax_id] = mito_code
...
with open("genetic_codes.tsv",'w') as mito_handle:
    for i, d in mito_code_lookup.items():
        mito_handle.writelines('{}\t{}\n'.format(i, d))
...
def Expand_taxonomy(tax_id, names_dictionary, nodes_dictionary):
    ...
    ## Add in taxonomic rank to the dataset.
    # format_tax = []
    # for i in range(len(final_tax)):
    #     format_tax.append('D_'+str(i)+'__'+final_tax[i])
    #combined_taxonomy = ';'.join(format_tax)

    combined_taxonomy = ';'.join(final_tax) # remove this id you want to add taxonomic rank
